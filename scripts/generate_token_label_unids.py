...
def main(
    ...
    if select_token_level == 'global': 
        ...
        unselected_labels = [[-100 for _ in range(len(label))] for label in raw_labels]
        selected_tokens_set = set(select_tokens_indices)
        
        for i, (sample_labels, sample_losses) in enumerate(zip(raw_labels, loss_diff)):
            for j, (label, loss) in enumerate(zip(sample_labels, sample_losses)):
                if label != -100 and (i, j) not in selected_tokens_set:
                    unselected_labels[i][j] = raw_labels[i][j]
        
        # 保存未被选中的token label矩阵
        unselected_data_path = os.path.join(label_path, f"noise_token_labels_{data_type}.pt")
        torch.save(unselected_labels, unselected_data_path)
        print(f"*** Unselected token-level label has been stored in {unselected_data_path} ***")
    ...
    final_data_path = os.path.join(label_path, f"token_labels_{data_type}.pt")
